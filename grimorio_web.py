...
# --- CSS DEFINITIVO (MOBILE-FIRST & ANTI DARK MODE) ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Crimson+Pro:wght@400;700&display=swap');
    
    /* Sfondo pergamena forzato */
    .stApp { 
        background-color: #fdf5e6 !important; 
    }
    
    /* Testo Nero Assoluto ovunque */
    .stApp, .stMarkdown, p, span, label, li, h1, h2, h3, h4, .stSelectbox p { 
    ...
    /* Fix per le caselle di selezione su Mobile */
    .stSelectbox div[data-baseweb="select"] {
        background-color: white !important;
        border-radius: 5px !important;
    }
    
    /* Nascondi elementi Streamlit per pulizia */
...
def pulizia_live(desc, durata):
    ...
    desc = desc.lstrip(' \t\n\r\f\v.,:;-')
    # Lista di parole della durata da rimuovere se appaiono all'inizio
    parti = re.findall(r'\w+', durata.lower()) + ["fino", "a", "ad", "ora", "ore", "minuto", "minuti", "round"]
    for _ in range(6): # Ripete per catturare frammenti multipli
...
# 1. BARRA DI RICERCA (Prioritaria)
nomi_tutti = sorted([s['name_it'] for s in spells])
scelta_search = st.selectbox("🔍 Cerca Incantesimo:", [""] + nomi_tutti, help="Inizia a scrivere per cercare...")
...
# --- LOGICA DI SELEZIONE ---
# Se la barra di ricerca non è vuota, visualizza quello. Altrimenti usa i filtri.
if scelta_search != "":
    nome_da_mostrare = scelta_search
else:
    nome_da_mostrare = scelta_lista

spell = next((s for s in spells if s['name_it'] == nome_da_mostrare), None)
...
if spell:
    ...
    if spell.get('higher_levels_it'):
        ...
        st.markdown(f"**Ai Livelli Superiori:** {spell['higher_levels_it']}")

else:
    st.write("Nessun incantesimo trovato.")